...
public interface SubscriptionsMapper {
    ...
    @Insert("INSERT INTO subscriptions(subscriptionId, tenant,namespace,topic,persistent,subscription) " +
            "VALUES(#{subscriptionId},#{tenant},#{namespace},#{topic},#{persistent},#{subscription})")
    void insert(SubscriptionEntity subscriptionsEntity);


    @Select("SELECT subscriptionId,tenant,namespace,topic,persistent,subscription FROM subscriptions" +
    ...
    @Select("SELECT subscriptionId,tenant,namespace,topic,persistent,partition,subscription FROM subscriptions " +
    ...
    @Update("UPDATE subscriptions set persistent=#{persistent}" +
            "WHERE tenant=#{tenant} and namespace=#{namespace} and topic=#{topic} and subscription=#{subscription}")
